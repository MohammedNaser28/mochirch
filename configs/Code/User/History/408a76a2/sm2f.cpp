...
void pewpew() {
    ...
    if ((n % 2 == 0 && odd != 0) || (n % 2 == 1 && odd != 1)) {
        cout << -1 << endl;
        debug("Impossible to form palindrome due to parity");
        return;
    }
    ...
    for (int i = 0; i < len; ++i) {
        for (int j = i + 1; j < len; ++j) {
            if (ans[i] != ans[j]) {
                ...
                debug2(ans[i], ans[j]);
                swap(ans[i], ans[j]);
                debug("After swap: " + ans);
                string rev = ans;
                reverse(all(rev));
                debug("rev: " + rev);
                if (ans == rev && ans > pal) {
                    debug("Found second palindrome: " + ans);
                    cout << ans << endl;
                    return;
                }
                swap(ans[i], ans[j]); 
    ...
    debug("No second palindrome found");
    cout << -1 << endl;
}
