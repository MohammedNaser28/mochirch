...
class Builder:
    ...
    CONFIGS_DIR = "configs"  # المجلد اللي فيه الإعدادات الشخصية

    def run(self) -> None:
        logger.success("The program has been launched successfully. We are starting the survey.")

        # فحص إن مجلد configs موجود
        if not os.path.exists(self.CONFIGS_DIR):
            logger.error(f"Configuration directory '{self.CONFIGS_DIR}' not found. Please create it and add your dotfiles.")
            exit(1)
        ...
        if self.build_options.make_backup:
            logger.info("The process of creating a backup of configurations is started!")
            FileSystemManager.make_backup()
            logger.warning(
                f"A backup of all your configuration files is located in './backup/'"
            )
        ...
        logger.warning("The script was unable to automatically install these packages. Try installing them manually.")
        logger.warning("Pacman: " + ", ".join(self.not_installed_packages.pacman))
        logger.warning("Aur: " + ", ".join(self.not_installed_packages.aur))
        logger.success("Your dotfiles have been successfully installed! Restart your PC to apply the changes.")
    ...
    def packages_installation(self) -> None:
        ...
        self.not_installed_packages.pacman.extend(PackageManager.install_packages(pacman))
        self.not_installed_packages.aur.extend(PackageManager.install_packages(aur, aur=self.build_options.aur_helper))
    ...
    def daemons_setting(self) -> None:
        ...
        for d in daemons["enable"]:
            try:
                ...
                logger.error(f'Error starting the "{d}" daemon: {traceback.format_exc()}')
