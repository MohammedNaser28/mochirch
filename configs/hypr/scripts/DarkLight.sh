...
# Paths
wallpaper_base_path="/home/mochi-002/Pictures/wallpapers"
dark_wallpapers="/home/mochi-002/Pictures/wallpapers/Dynamic-Wallpapers/Dark"
light_wallpapers="/home/mochi-002/Pictures/wallpapers/Dynamic-Wallpapers/Light"
...
vscode_settings_oss="$HOME/.config/Code - OSS/User/settings.json"
vscode_settings_codium="$HOME/.config/VSCodium/User/settings.json"
...
# Function to set VS Code theme
set_vscode_theme() {
    mode=$1
    for settings_file in "$vscode_settings" "$vscode_settings_oss" "$vscode_settings_codium"; do
        if [ -f "$settings_file" ]; then
            if [ "$mode" = "Dark" ]; then
                jq '."workbench.colorTheme" = "Alt Catppuccin Mocha"' "$settings_file" > "${settings_file}.tmp" && mv "${settings_file}.tmp" "$settings_file"
            else
                jq '."workbench.colorTheme" = "Alt Catppuccin Latte"' "$settings_file" > "${settings_file}.tmp" && mv "${settings_file}.tmp" "$settings_file"
            fi
            # Keep title bar style consistent
            jq '."window.titleBarStyle" = "custom"' "$settings_file" > "${settings_file}.tmp" && mv "${settings_file}.tmp" "$settings_file"
        fi
    done
}
...
# Call the functions after determining the mode
set_waybar_style "$next_mode"
set_vscode_theme "$next_mode"
